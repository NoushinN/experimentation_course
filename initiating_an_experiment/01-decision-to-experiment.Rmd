...
Every positive side comes with a negative one, and so no design is really perfect. This is not to discourage us. In a way, George Box's (British statistician) quote that "All models are wrong, but some are useful" applies to experimental designs as well. One single experimental desgin cannot be comprehensive and all-encompassing. Additionally, we cannot control for the many factors and behaviours that may affect a situation. 
...
For instance, we may decide to base our tests on a set of observations derived from survey findings and not be aware that survey data can badly fail in two aspects: 
...
Additionally, statistical models that are used to analyse collected data also work under assumptions. For example, a simple linear regression model requires four assumptions:
...
Without checking these assumptions, we are using the wrong models and generating misguided insights or misinformation. 
...
To summarise, experiments can be 'dangerous' if we fall into the following pitfalls (not an exhastive list):
...
- Ingore errors and outliers
- Not consider the ethical issues and scenarios or have informed consent from participants


## It is not all bad

Experimentation is not all bad news. Many breakthroughs and transformations on many fronts from medicine and technology to social changes for good have come about by a willingness to experiment. 

<center>
![](fig/cellphone_evolution.png){width=80%}
</center>

There are definitely many pros to experimentation. For instance, experiments result in a higher degree of internal validity. Through random assignment of a treatment condition, experimental designs allow us to examine the effect of one variable while keeping other conditions constant. Note that randomization is the key here because it ensures that the treatment and control groups are comparable. Any differences between the two groups can be attributed to the treatment. This is great to know and invest on as a business.

Pro 2 It’s Good Even Without Prior Knowledge
As stated, the observational study depends on theories to guide variable selection and falls short if there is no theory to start. Emerging research areas run into this problem frequently.
Since it directly controls how data is generated, the experimental approach can survive and thrive with no previous knowledge.
With no prior knowledge, electoral researchers carry out a field experiment to examine how different ways of contacting voters would affect the voting turnout.
Pro3 Experiments Clarify Mixed Results
Due to different measurements and data sources, observational studies often generate mixed results.
How can we clarify the mixed results?
Run an experiment!
Again in electoral politics, researchers run an experiment to detect how campaign spending affects the voters for the incumbent and the voters for the challengers differently.
